...
import ProgressBar from "$lib/util/ProgressBar.svelte";
import { onMount } from "svelte";
import { formatDataVolume } from "$lib/util/Formatting.svelte";
import { session } from "$app/stores";
import { api_endpoint } from "$lib/Constants.svelte";

let loading = false
let subscription = $session.user.subscription.id
let hotlinking = $session.user.hotlinking_enabled
let transfer_cap = $session.user.monthly_transfer_cap / 1e9
...
const update = async (update_field) => {
	...
	try {
		const resp = await fetch(
			api_endpoint+"/user/subscription",
