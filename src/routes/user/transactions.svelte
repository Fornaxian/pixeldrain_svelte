...
import { formatDataVolume, formatDate } from "$lib/util/Formatting.svelte";
	...
	try {
		...
		while (true) {
			const resp = await fetch(
				api_endpoint+"/user/transactions/" +
...
const load_invoices = async () => {
	loading = true
	try {
		const resp = await fetch(api_endpoint+"/btcpay/invoice")
		if(resp.status >= 400) {
